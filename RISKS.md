...
Risk mitigation is not allowed to become onboarding tax. If a mitigation asks the user to read vendor docs, create a vendor account before they can try us, compare plans, configure IAM, or paste five secrets into their project, it is not a mitigation. It is product failure wearing a safety vest.
...
| 8 | LLM master-key traffic creates safety and liability exposure | High | Put moderation, rate limits, logging minimization, and vendor-policy flowdown in the gateway. Route risky use cases to Connected Providers or deny them. Keep the user API unchanged; change only the backend route or error reason. |
...
| 10 | Connected-provider custody creates secret-management obligations | High | Treat BYOK and OAuth-connected providers as paid graduation features backed by envelope encryption, scoped decrypt paths, audit logs, rotation, redaction, and emergency purge. The user's project still receives only `CARAVANSARY_API_KEY`; the operational burden stays with us. |
...
| 14 | Partner programs take longer than expected | Medium | Ship Phase 0 on modes that do not require contracts: self-hosted OSS and permitted master-key use. Keep BYOK as a connected-provider graduation feature, not as the default free-tier path. Design provider adapters so partner provisioning can replace master-key routes later without changing the public API. |
...
| 17 | The product drifts into enterprise workflow software | Medium | Keep enterprise features behind graduation: teams, audit, SSO, custom DPA, region pinning. None of them enters the first-run path. The first-run path remains sign in, initialize or copy key, call endpoint. |
...
10. Does this strengthen or weaken "sign in once, ship"?
